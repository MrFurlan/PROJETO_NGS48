...
#Region "Construtor"
    ...
    Public Sub New(ByVal nomeArquivo As String)
        Dim sql As String = "SELECT TOP 1 codresp, texto, nomearquivoresp FROM nfe.resp WHERE nomearquivoresp = '" & nomeArquivo & "'"
        Dim db As New AcessaBanco
        Dim ds As DataSet = db.ConsultaDataSet(sql, "resp")
        If ds IsNot Nothing AndAlso ds.Tables IsNot Nothing AndAlso ds.Tables.Count > 0 AndAlso ds.Tables(0).Rows.Count > 0 Then
            ...
            Me.Codigo = row("codresp")
            Me.NomeArquivo = row("nomearquivoresp")
            Me.Texto = row("texto")
        End If
    End Sub
#End Region
        ...
        Select Case Me.IUD
            Case "I"
                ...
                Sql = "INSERT INTO NFE.FIL" & vbCrLf & _
                      "     (TEXTO, NOMEARQUIVORESP) " & vbCrLf & _
                      " VALUES " & vbCrLf & _
                      "     ('" & _Texto & "', '" & _NomeArquivo & "') "
                Sqls.Add(Sql)
            Case "U"
                Sql = " UPDATE NFE.FIL" & vbCrLf & _
                      ...
                      "         NOMEARQUIVORESP  = '" & _NomeArquivo & "'" & vbCrLf & _
                      "     WHERE CODRESP = " & _Codigo
                Sqls.Add(Sql)
            Case "D"
