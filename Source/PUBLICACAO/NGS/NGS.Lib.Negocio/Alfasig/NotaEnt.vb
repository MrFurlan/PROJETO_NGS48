...
#Region "Construtor"
    ...
    Public Sub New(ByVal nomeArquivo As String)
        Dim sql As String = "SELECT * FROM nfe.notaent WHERE nomearquivo = '" & nomeArquivo & "'"
        Dim db As New AcessaBanco
        Dim ds As DataSet = db.ConsultaDataSet(sql, "notaent")
        If ds IsNot Nothing AndAlso ds.Tables IsNot Nothing AndAlso ds.Tables.Count > 0 AndAlso ds.Tables(0).Rows.Count > 0 Then
            ...
            Me.Codigo = row("codnotaent")
            Me.NomeArquivo = row("nomearquivo")
            Me.Texto = row("texto")
        End If
    End Sub
#End Region
        ...
        Select Case Me.IUD
            ...
            Case "U"
                Sql = " UPDATE NFE.FIL" & vbCrLf & _
                      ...
                      "         NOMEARQUIVO  = '" & _NomeArquivo & "'" & vbCrLf & _
                      "     WHERE CODNOTAENT = " & _Codigo
                Sqls.Add(Sql)
            Case "D"
