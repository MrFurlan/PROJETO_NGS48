...
namespace DocsBr.Validation.IE
{
    ...
    public class IETocantinsValidator : IIEValidator
    {
        ...
        private bool HasValid3rdAnd4thDigits()
        {
            ...
            {
                string[] validNumbersIEOutros = {
                                        "1", // Produtor Rural (não possui CGC)
                                        "2", // Industria e Comércio
                                        "3", // Empresas Rudimentares
                                        "9", // Empresas do Cadastro Antigo (SUSPENSAS)
                                    };
                return validNumbersIEOutros.Contains(this.inscEstadual.Substring(1, 1));
            }
