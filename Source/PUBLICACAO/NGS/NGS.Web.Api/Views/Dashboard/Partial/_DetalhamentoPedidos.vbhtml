...
@Code
    ...
    Dim totalClientes As Integer = Model.Count
    Dim totalCidades As Integer = Model.Select(Function(p) p.Cidade).Distinct().Count()
    Dim totalEstados As Integer = Model.Select(Function(p) p.Estado).Distinct().Count()
End Code

<h2>Detalhes dos Pedidos (Top 10)</h2>
    ...
    <tbody>
        @For Each produto In Model
            @<tr>
                ...
                <td>@produto.ClienteFantasia</td>
                <td>@produto.Data</td>
                <td>@produto.QuantidadeFiscal.ToString("N0")</td>
                <td>@produto.PesoFiscal.ToString("N0")</td>
...
<script>
    $(function () {
        $('#tabelaProdutos').DataTable({
            ...
            "order": [[5, "desc"]], // Ordena pela 6ª coluna ("Valor Total"), ordem decrescente
            "language": {
