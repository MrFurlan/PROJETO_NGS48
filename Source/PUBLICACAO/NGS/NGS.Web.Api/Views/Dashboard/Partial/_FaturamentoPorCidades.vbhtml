...
"#316395"  '#10
    ...
    Dim chartDataPorCidade = New With {
        .labels = Model.Select(Function(p) p.Cidade).ToArray(),
        .datasets = New Object() {
            New With {
                .label = "Faturamento (R$)",
                .data = Model.Select(Function(p) p.ValorTotal).ToArray(),
...
<script type="application/json" id="chartDataPorCidade">
    @Html.Raw(Json.Encode(chartDataPorCidade))
</script>

<!-- Canvas para renderização do gráfico -->
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Valor Total por Cidade (Top 10)</h3>
    </div>
    <div class="card-body">
        <canvas id="faturamentoCidade"></canvas>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        try {
            ...
            const data = JSON.parse(document.getElementById('chartDataPorCidade').textContent);

            // Verificação para debug (opcional)
            console.log("Dados carregados:", data);

            // Inicializar o gráfico
            const ctx = document.getElementById('faturamentoCidade').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: data,
                options: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: function (tooltipItem) {
                                    ...
                                    const nomeProduto = @Html.Raw(Json.Encode(Model.Select(Function(p) p.Nome).ToArray()));
