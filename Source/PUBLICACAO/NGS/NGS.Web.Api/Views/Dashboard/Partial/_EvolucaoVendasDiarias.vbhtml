...
@Code
    ...
    Dim chartDataEvolucaoVendasDiarias = New With {
...
<script type="application/json" id="chartDataEvolucaoVendasDiarias">
    @Html.Raw(Json.Encode(chartDataEvolucaoVendasDiarias))
</script>

<!-- Canvas para renderização do gráfico -->
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Evolução de Vendas Diárias</h3>
    </div>
    <div class="card-body">
        <canvas id="evolucaoVendasDiarias"></canvas>
    </div>
</div>
...
<script>
    $(document).ready(function () {
        try {
            ...
            const data = JSON.parse(document.getElementById('chartDataEvolucaoVendasDiarias').textContent);
            ...
            const ctx = document.getElementById('evolucaoVendasDiarias').getContext('2d');
            new Chart(ctx, {
                type: 'line',
                data: data,
                options: {
