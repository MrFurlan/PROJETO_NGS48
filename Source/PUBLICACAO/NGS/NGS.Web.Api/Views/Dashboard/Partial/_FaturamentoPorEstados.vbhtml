...
"#316395"  '#10
    ...
    Dim chartDataPorEstado = New With {
        .labels = Model.Select(Function(p) p.Estado).ToArray(),
        .datasets = New Object() {
            New With {
                .label = "Faturamento (R$)",
...
<script type="application/json" id="chartDataPorEstado">
    @Html.Raw(Json.Encode(chartDataPorEstado))
</script>

<!-- Canvas para renderização do gráfico -->
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Valor Total por Estado (Top 10)</h3>
    </div>
    <div class="card-body">
        <canvas id="faturamentoEstado"></canvas>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        try {
            ...
            const data = JSON.parse(document.getElementById('chartDataPorEstado').textContent);

            // Verificação para debug (opcional)
            console.log("Dados carregados:", data);

            // Inicializar o gráfico
            const ctx = document.getElementById('faturamentoEstado').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: data,
                options: {
