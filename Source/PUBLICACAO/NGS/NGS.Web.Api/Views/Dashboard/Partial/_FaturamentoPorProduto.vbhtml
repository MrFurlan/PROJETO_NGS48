...
<script type="application/json" id="chartData">
    @Html.Raw(Json.Encode(chartData))
</script>

<!-- Canvas para renderização do gráfico -->
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Valor Total por Produto</h3>
    </div>
    <div class="card-body">
        <canvas id="faturamentoProduto"></canvas>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function () {
        try {
            ...
            const ctx = document.getElementById('faturamentoProduto').getContext('2d');
            new Chart(ctx, {
                type: 'bar',
                data: data,
                options: {
