...
Public Class ucConsultaPedido
    ...
    Public Sub BindGridView(ByVal parameters As Dictionary(Of String, Object))
        ...
        Sql &= "SELECT P.Empresa_Id, P.EndEmpresa_Id, Empresa.Nome, P.Safra, " & vbCrLf & _
               ...
               "  Left Join (" & vbCrLf & _
               "             Select PP.EmpresaOrigem_Id," & vbCrLf & _
               "                    PP.EndEmpresaOrigem_Id," & vbCrLf & _
               "                    PP.PedidoOrigem_Id," & vbCrLf & _
               "                    PP.SequenciaOrigem_Id," & vbCrLf & _
               "                    Sum(PxE2.ValorOficial) as OficialDestino," & vbCrLf & _
               "                    Sum(PxE2.ValorMoeda) as MoedaDestino" & vbCrLf & _
               "               from PedidoxPedido PP" & vbCrLf & _
               "              inner Join PedidosxEncargos PxE2" & vbCrLf & _
               "                 on PP.EmpresaDestino_Id    = PxE2.Empresa_Id" & vbCrLf & _
               "                And PP.EndEmpresaDestino_Id = PxE2.EndEmpresa_Id" & vbCrLf & _
               "                And PP.PedidoDestino_id     = PxE2.Pedido_Id" & vbCrLf & _
               "                and PxE2.Encargo_Id         = 'LIQUIDO'" & vbCrLf & _
               "              Group by PP.EmpresaOrigem_Id, PP.EndEmpresaOrigem_Id, PP.PedidoOrigem_Id, PP.SequenciaOrigem_Id" & vbCrLf & _
               "            ) as sb" & vbCrLf & _
               "     on P.Empresa_id      = sb.EmpresaOrigem_Id" & vbCrLf & _
               "    and P.EndEmpresa_Id   = sb.EndEmpresaOrigem_Id" & vbCrLf & _
               "    and P.Pedido_Id       = sb.PedidoOrigem_Id" & vbCrLf & _
        ...
        Sql &= "  group by P.Empresa_Id, P.EndEmpresa_Id, Empresa.Nome, P.Safra, " & vbCrLf & _
               ...
               " Order By DataPedido Desc " & vbCrLf & _
    ...
    Protected Sub dgPedido_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim Ped As New [Lib].Negocio.Pedido(dgPedido.SelectedRow.Cells(1).Text, dgPedido.SelectedRow.Cells(2).Text, dgPedido.SelectedRow.Cells(6).Text)
        dgItens.DataSource = Ped.Itens
        dgItens.DataBind()
    End Sub

    Protected Overrides Sub Selecionar()
        Try
            Dim objPedido As New [Lib].Negocio.Pedido(dgPedido.SelectedRow.Cells(1).Text, dgPedido.SelectedRow.Cells(2).Text, dgPedido.SelectedRow.Cells(6).Text)
            ...
            If Session("ssTipoRetorno") IsNot Nothing Then
                ...
                End If
                Popup.CloseDialog(Me.Page, "divConsultaPedido")
            End If
