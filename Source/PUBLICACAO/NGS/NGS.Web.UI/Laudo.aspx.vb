...
#Region "Page_Load"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsConnect AndAlso Not IsPostBack Then
            ...
            If Funcoes.VerificaPermissao("LaudoDePesagem", "ACESSAR") Then
                CarregarBalanca()
                IncluirEventos()
                BuscarSituacao()
...
#Region "Botões"
    ...
    Protected Sub btnIncluir_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnIncluir.Click
        Try
            If Funcoes.VerificaPermissao("LaudoDePesagem", "GRAVAR") Then
    ...
    Protected Sub btnConsultar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnConsultar.Click
        If Funcoes.VerificaPermissao("LaudoDePesagem", "LEITURA") Then
            If (String.IsNullOrEmpty(txtLaudo.Text)) Then
                CarregarGridLaudo()
            Else
    ...
    Protected Sub btnReimprimir_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnReimprimir.Click
        If Funcoes.VerificaPermissao("LaudoDePesagem", "RELATORIO") Then
            SessaoRecuperaLaudo()
            If objLaudo.SegundaPesagem > 0 AndAlso objLaudo.Analises IsNot Nothing Then
    ...
    Protected Sub btnAutorizacao_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAutorizacao.Click
        ...
        If Not String.IsNullOrWhiteSpace(txtSegundaPesagem.Text) AndAlso _
            ...
            If gridDescontos.Rows.Count > 0 Then
                ...
                End If
            Else
                If objLaudo.Produto.Agrupar = "S" Then
    ...
    Protected Sub btnCancelar_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancelar.Click
        If Funcoes.VerificaPermissao("LaudoDePesagem", "EXCLUIR") Then
            SessaoRecuperaLaudo()

            If objLaudo.TemNota Then
                MsgBox(Me.Page, "Pesagem com Nota Fiscal não pode ser cancelada.")
            ElseIf (String.IsNullOrWhiteSpace(txtObservacao.Text) OrElse txtObservacao.Text.Trim.Length < 15) Then
    ...
    Protected Sub imgExtratoPedido_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles imgExtratoPedido.Click
        Extrato.Emitir(Me.Page, FinanceiroNovo, txtCnpjDaEmpresa.Text.Replace(".", "").Replace("/", "").Replace("-", ""), hdnCodigoEndEmpresa.Value, "T",
                         txtPedido.Text, "", 0, "", Nothing, "", False, False, False, False, False, False, False, True)
    End Sub
    ...
    Protected Sub ddlSubOperacao_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlSubOperacao.SelectedIndexChanged
        If ddlSubOperacao.SelectedIndex > 0 Then
            ...
            If sOp(1) > 0 Then
                ...
                objLaudo.CodigoPedido = txtPedido.Text
                objLaudo.Consolidado = chkConsolidarCliente.Checked
...
#Region "Carregar"
    ...
    Public Overrides Sub Carregar(ByVal obj As [Lib].Negocio.IBaseEntity)
        ...
        ElseIf Not objLaudo Is Nothing Then
            Select Case hdnControlePopup.Value.Replace(",", "")
                Case "Cliente"
                    objLaudo.Cliente = CType(obj, [Lib].Negocio.Cliente)
                    With objLaudo.Cliente
                        objLaudo.CodigoCliente = .Codigo
                        objLaudo.EnderecoCliente = .CodigoEndereco
                        ...
                        If (Not String.IsNullOrEmpty(.Telefone)) Then
                            objLaudo.Cliente.Telefone = objLaudo.Cliente.Telefone.Replace("(", "").Replace(")", "").Replace("-", "").Replace(" ", "")
                            txtTelefoneDoCliente.Text = String.Format("{0:(##) ####-####}", Long.Parse(objLaudo.Cliente.Telefone.Trim))
                        End If
                        ...
                        objLaudo.CodigoDepositante = .Codigo
                        objLaudo.EnderecoDepositante = .CodigoEndereco
                        Limpar()
                        CarregarPopupPedido(True)
                    End With
                Case "Transportador"
                ...
                Case "Depositante"
                    objLaudo.Depositante = CType(obj, [Lib].Negocio.Cliente)
                    With objLaudo.Depositante
                        objLaudo.CodigoDepositante = .Codigo
                        objLaudo.EnderecoDepositante = .CodigoEndereco
                        txtDepositante.Text = .CodigoFormatado & "-" & .Nome
                        hdnCodigoEndDepositante.Value = .CodigoEndereco
                ...
                Case "Deposito"
                    objLaudo.Deposito = CType(obj, [Lib].Negocio.Cliente)
                    With objLaudo.Deposito
                        objLaudo.CodigoDeposito = .Codigo
                        objLaudo.EnderecoDeposito = .CodigoEndereco
                    ...
                    objLaudo.Pedido = New [Lib].Negocio.Pedido(Empresa, Endereco, Pedido)
                    hdnControlePopup.Value = "Transportador"
                    ucConsultaClientesDireto.Limpar()
                    ucConsultaClientesDireto.SetarTituloDIV("Consulta de Transportador")
                    ucConsultaClientesDireto.CarregarTransportadoresPorPedido(objLaudo.Pedido.Transportadores)
                    Popup.ConsultaDeClientesDireto(Me.Page, "objLaudo" & HID.Value, "", True, 600)
                    Popup.CenterDialog(Me.Page, "divConsultaClienteDireto")
                Case "Pedido"
                    objLaudo.Pedido = CType(Session("objLaudo" & HID.Value), [Lib].Negocio.Pedido)
                    objLaudo.CodigoPedido = objLaudo.Pedido.Codigo
                    imgExtratoPedido.Visible = True
                    If objLaudo.CodigoPedido = 0 Then
                    ...
                    Else
                        ...
                        If EmbarqueNovo Then
                            lstAutCarregamento = New ListAutCarregamento(objLaudo.CodigoPedido, objLaudo.CodigoPlaca)
                            If lstAutCarregamento IsNot Nothing AndAlso lstAutCarregamento.Count > 0 Then
                                ...
                                For Each objAutCarregamento As [Lib].Negocio.AutCarregamento In lstAutCarregamento
                                    Dim objPedidoXTransportador As New [Lib].Negocio.PedidoXTransportador(objLaudo.Pedido)
                                    ...
                                    objLaudo.Pedido.Transportadores.Add(objPedidoXTransportador)
                                Next
                            End If
                        Else
                            'PESQUISA OS TRANSPORTADORES DO PEDIDO
                            objLaudo.Pedido.Transportadores = New [Lib].Negocio.ListPedidoXTransportador(objLaudo.Pedido)
                        End If

                        'PESQUISA OS ITENS DO PEDIDO
                        'objLaudo.Pedido.Itens = New [Lib].Negocio.ListPedidoXItem(objLaudo.Pedido)

                        'PESQUISA OS DEPÓSITOS DO PEDIDO
                        objLaudo.Pedido.Depositos = New [Lib].Negocio.ListPedidoxDeposito(objLaudo.Pedido)

                        With objLaudo.Pedido
                            objLaudo.CodigoPedido = .Codigo
                            objLaudo.CodigoProduto = .Itens(0).CodigoProduto
                            objLaudo.CodigoOperacao = .CodigoOperacao
                            objLaudo.CodigoSubOperacao = .CodigoSubOperacao
                            ...
                            If String.IsNullOrWhiteSpace(txtPrimeiraPesagem.Text) OrElse Convert.ToInt32(txtPrimeiraPesagem.Text) = 0 Then
                                If Left(objLaudo.CodigoEmpresa, 8) = "04854422" Then ddlTabelaDeClassificacao.Enabled = True
                            End If
                            '*****************************************************************************************************

                            objLaudo.CodigoTabelaDeClassificacao = .Itens(0).Classificacao.Codigo
                            ...
                            End If
                            If objLaudo.Pedido.SubOperacao.EntradaSaida = eEntradaSaida.Entrada Then
                                lblEntradaSaida.Text = "ENTRADA"
                            Else
                                lblEntradaSaida.Text = "SAIDA"
                            End If
                            ...
                            objLaudo.Consolidado = chkConsolidarCliente.Checked
                            Dim ds As DataSet = objLaudo.SaldoDePedidos(objLaudo, False)
                            hdnSaldoPedido.Value = ds.Tables(0).Rows(0).Item("Saldo")
                            txtSaldoPedido.Text = hdnSaldoPedido.Value
                            ddlOperacao.Enabled = False
                        End With
                        Dim deposito As [Lib].Negocio.PedidoXDeposito = objLaudo.Pedido.Depositos.FirstOrDefault(Function(D) D.Tipo = "DE")
                        ...
                        If deposito IsNot Nothing Then
                            ...
                            objLaudo.CodigoDeposito = deposito.Deposito.Codigo
                            objLaudo.EnderecoDeposito = deposito.Deposito.CodigoEndereco
                            'Transportador
                            If (objLaudo.Pedido.Transportadores.Count.Equals(1)) Then
                                txtTransportador.Text = objLaudo.Pedido.Transportadores(0).Transportador.CodigoFormatado & "-" & objLaudo.Pedido.Transportadores(0).Transportador.Nome
                                hdnCodigoEndTransportador.Value = objLaudo.Pedido.Transportadores(0).Transportador.CodigoEndereco
                                objLaudo.CodigoTransportador = objLaudo.Pedido.Transportadores(0).Transportador.Codigo
                                objLaudo.EnderecoTransportador = objLaudo.Pedido.Transportadores(0).Transportador.CodigoEndereco
                                If EmbarqueNovo Then
                                    If lstAutCarregamento IsNot Nothing AndAlso lstAutCarregamento.Count > 0 AndAlso Not String.IsNullOrWhiteSpace(lstAutCarregamento(0).Placa) Then
                                        objLaudo.Placa = New [Lib].Negocio.Placa(lstAutCarregamento(0).Placa)
                                        With objLaudo.Placa
                                            objLaudo.CodigoPlaca = .Placa01
                                            objLaudo.CodigoViaDeTransporte = .ViaDeTransporte
                                            objLaudo.CodigoMotorista = .Motorista.Codigo
                                ...
                                Else
                                    btnPlaca_Click(btnPlaca, Nothing)
                                End If
                            ElseIf (objLaudo.Pedido.Transportadores.Count.Equals(0)) Then
                            ...
                            Else
                                ...
                                ucConsultaClientesDireto.SetarTituloDIV("Consulta de Transportador")
                                ucConsultaClientesDireto.CarregarTransportadoresPorPedido(objLaudo.Pedido.Transportadores)
                                Popup.ConsultaDeClientesDireto(Me.Page, "objLaudo" & HID.Value)
                                Popup.CenterDialog(Me.Page, "divConsultaClienteDireto")
                            End If
                            CarregarAnalises()
                        Else
                            ...
                            ucConsultaClientesDireto.CarregarDepositosPorPedido(objLaudo.Pedido.Depositos)
                            Popup.ConsultaDeClientesDireto(Me.Page, "objLaudo" & HID.Value)
                            Popup.CenterDialog(Me.Page, "divConsultaClienteDireto")
                        End If
                    End If
    ...
    Protected Sub CarregarSubOperacao()
        ...
        Dim Where As String
        Where = "     So.Operacao_id =" & ddlOperacao.SelectedValue & vbCrLf & _
                " AND So.Laudo = 'S'" & vbCrLf & _
                " AND So.Situacao = 1" & vbCrLf & _
                " AND So.Liminar = " & IIf(objLaudo.Pedido.SubOperacao.Liminar, 1, 0) & vbCrLf & _
                " AND So.Consignacao =  " & IIf(objLaudo.Pedido.SubOperacao.Consignacao, 1, 0) & vbCrLf & _
                " AND So.PrecoFixo = '" & IIf(objLaudo.Pedido.SubOperacao.PrecoFixo, "S", "N") & "'" & vbCrLf
    ...
    Protected Sub CarregarAnalises()
        ...
        objLaudo.CodigoTabelaDeClassificacao = ddlTabelaDeClassificacao.SelectedValue
        objLaudo.CodigoProduto = txtProduto.Text.Split("-").ToArray(0).TrimEnd
        objLaudo.Analises = New [Lib].Negocio.ListPesagemXAnalises(objLaudo)

        btnCalcular.Visible = objLaudo.Analises IsNot Nothing AndAlso objLaudo.Analises.Count > 0
    ...
    Protected Sub CarregarGridLaudo()
        objLaudo = New [Lib].Negocio.Pesagem
        objLaudo.CodigoEmpresa = txtCnpjDaEmpresa.Text.Replace(".", "").Replace("/", "").Replace("-", "")
        objLaudo.EnderecoEmpresa = hdnCodigoEndEmpresa.Value
        Dim listPesagem As New ListPesagem(objLaudo, False, txtMovimento.Text, txtMovimento.Text)
        gdvLaudo.DataSource = listPesagem
        gdvLaudo.DataBind()

        For i = 0 To gdvLaudo.Rows.Count - 1
            If gdvLaudo.Rows(i).Cells(1).Text = listPesagem(i).Codigo AndAlso Not listPesagem(i).CodigoSituacao = 1 Then
    ...
    Public Sub CarregarPopupPedido(ByVal AnoCorrente As Boolean)
        ...
        Popup.ConsultaDePedidos(Me.Page, "objLaudo" & HID.Value)
        hdnControlePopup.Value = "Pedido"
    End Sub
    ...
    Public Sub CarregarAutorizacaoDeRetirada()
        ...
        Popup.ConsultaDeAutorizacaoDeRetirada(Me.Page, "objLaudo" & HID.Value)
    End Sub

    Public Sub CarregarAutorizacao(Par As Hashtable)
        Dim sOp() As String = ddlSubOperacao.SelectedValue.ToString.Split("-")
        Dim suboperacao As New [Lib].Negocio.SubOperacao(sOp(0), sOp(1))
        Dim Autorizacao As New [Lib].Negocio.AutorizacaoDeRetirada(txtCnpjDaEmpresa.Text.Replace(".", "").Replace("-", "").Replace("/", ""), hdnCodigoEndEmpresa.Value, Par("Pedido"), Par("Autorizacao"), suboperacao.Classe)
...
#End Region
    ...
    Protected Sub Salvar(ByVal IUD As String)
        ...
        objLaudo.EntradaPatio = DateTime.Now
        ...
        If (IUD.ToString.Equals("I")) Then
            ...
            For i = 0 To gridDescontos.Rows.Count - 1
                Dim objPesagemXAnalises As New PesagemXAnalises(objLaudo)
                ...
                If Len(objPesagemXAnalises.Analise.Opcao) > 0 Then
                    ...
                    objLaudo.Analises.Add(objPesagemXAnalises)
                Else
        ...
        objLaudo.CodigoAutorizacao = hdnAutorizacaoDeRetirada.Value
        'Produto
        objLaudo.CodigoProduto = txtProduto.Text.Split("-").ToArray(0).Trim
        'Cliente
        objLaudo.CodigoCliente = txtCnpjDoCliente.Text.Replace(".", "").Replace("/", "").Replace("-", "")
        objLaudo.EnderecoCliente = hdnCodigoEndCliente.Value
        'Depositante
        objLaudo.CodigoDepositante = txtDepositante.Text.Split("-").ToArray(0).Trim.Replace(".", "").Replace("/", "").Replace("-", "") & txtDepositante.Text.Split("-").ToArray(1).Trim
        objLaudo.EnderecoDepositante = hdnCodigoEndDepositante.Value
        ...
        'Deposito
        objLaudo.CodigoDeposito = txtDeposito.Text.Split("-").ToArray(0).Trim.Replace(".", "").Replace("/", "").Replace("-", "") & txtDeposito.Text.Split("-").ToArray(1).Trim
        objLaudo.EnderecoDeposito = hdnCodigoEndDeposito.Value
        'Tabela Classificação
        objLaudo.CodigoTabelaDeClassificacao = ddlTabelaDeClassificacao.SelectedValue.ToString
        'Operação
        objLaudo.CodigoOperacao = ddlOperacao.SelectedValue.ToString
        'SubOperação
        objLaudo.CodigoSubOperacao = ddlSubOperacao.SelectedValue.Split("-").ToArray(1).Trim
        'Pesagem.
        ...
        If Not String.IsNullOrEmpty(txtLaudo.Text) Then
            If (Convert.ToInt32(txtPrimeiraPesagem.Text) > Convert.ToInt32(txtSegundaPesagem.Text)) Then
                objLaudo.EntradaSaida = "E" 'Entrada
            Else
                objLaudo.EntradaSaida = "S" 'Saida
            End If
            objLaudo.SaidaBalanca = DateTime.Now 'Data da segunda pesagem.
        Else
            If objLaudo.SubOperacao.EntradaSaida = eEntradaSaida.Entrada Then
                objLaudo.EntradaSaida = "E" 'Entrada
            Else
                objLaudo.EntradaSaida = "S" 'Saida
            End If
        ...
        If (Not String.IsNullOrEmpty(txtNotaProdutor.Text)) Then
            objLaudo.NumeroDaNota = txtNotaProdutor.Text
            objLaudo.SerieDaNota = txtSerieNotaProdutor.Text
            objLaudo.PesoFiscal = txtPesoNotaProdutor.Text
        End If
    ...
    Protected Function ValidarCampos(ByVal IUD As String) As String
        ...
        If (Not String.IsNullOrWhiteSpace(txtSegundaPesagem.Text) AndAlso Convert.ToInt32(txtSegundaPesagem.Text) > 0) Then
            If Convert.ToInt32(txtPrimeiraPesagem.Text) > Convert.ToInt32(txtSegundaPesagem.Text) AndAlso lblEntradaSaida.Text = "SAIDA" Then
                Erro += "Operação de saída não pode ser usada para Entrada \n"
            ElseIf Convert.ToInt32(txtSegundaPesagem.Text) > Convert.ToInt32(txtPrimeiraPesagem.Text) AndAlso lblEntradaSaida.Text = "ENTRADA" Then
            ...
            ElseIf lblEntradaSaida.Text = "SAIDA" AndAlso hdnAutorizacaoDeRetirada.Value = 0 Then
                ...
                If objPrd.Agrupar = "N" AndAlso Not sOp(0) = "70" Then Erro += "Autorização de Retirada não foi selecionada \n"
            End If
        End If
        ...
        If Not String.IsNullOrWhiteSpace(txtSegundaPesagem.Text) AndAlso Convert.ToInt32(txtSegundaPesagem.Text) > 0 Then
            ...
            If Convert.ToInt32(txtPesoBruto.Text) = Convert.ToInt32(txtLiquido.Text) AndAlso pDesconto > 0 Then
        ...
        'Saldo
        If objLaudo.Produto.ControlarRomaneio Then
            If Not String.IsNullOrWhiteSpace(txtSegundaPesagem.Text) AndAlso (Convert.ToInt32(txtSegundaPesagem.Text) > 0) Then
                Dim sOp() As String = ddlSubOperacao.SelectedValue.ToString.Split("-")
                Dim suboperacao As New [Lib].Negocio.SubOperacao(sOp(0), sOp(1))

                If Not suboperacao.Devolucao AndAlso suboperacao.EntradaSaida = eEntradaSaida.Entrada AndAlso (objLaudo.Pedido.SubOperacao.Classe = eClassesOperacoes.AFIXAR OrElse objLaudo.Pedido.SubOperacao.Classe = eClassesOperacoes.DEPOSITOS) Then
                    'PODE PASSAR SEM CHECAR O SALDO
                Else
                    If Not suboperacao.Devolucao AndAlso (Convert.ToInt32(txtLiquido.Text) > Convert.ToInt32(hdnSaldoPedido.Value)) Then
    ...
    Protected Sub CarregarLaudo(ByVal Pesagem As String, ByVal Empresa As String, ByVal Endereco As String)
        LimparCampos()
        objLaudo = New Pesagem(Empresa, Endereco, Pesagem)
        txtLaudo.Text = Pesagem
        SessaoSalvaLaudo()
        If (objLaudo.Empresa IsNot Nothing And objLaudo.Cliente IsNot Nothing) Then
            ...
            Else
                ...
                btnReimprimir.Enabled = False
            End If
            If objLaudo.Romaneios.Count = 0 Then
                txtRomaneio.Text = ""
            ElseIf objLaudo.Romaneios.Count = 1 Then
            ...
            'Cliente
            txtNomeDoCliente.Text = objLaudo.Cliente.Nome
            txtCnpjDoCliente.Text = objLaudo.Cliente.CodigoFormatado
            hdnCodigoEndCliente.Value = objLaudo.Cliente.CodigoEndereco
            txtEnderecoDoCliente.Text = objLaudo.Cliente.Endereco
            txtComplementoDoCliente.Text = objLaudo.Cliente.Complemento
            txtBairroDoCliente.Text = objLaudo.Cliente.Bairro
            txtCepDoCliente.Text = objLaudo.Cliente.CEP
            txtMunicipioDoCliente.Text = objLaudo.Cliente.Cidade
            txtTelefoneDoCliente.Text = objLaudo.Cliente.Telefone
            txtEstadoDoCliente.Text = objLaudo.Cliente.Estado.Codigo
            txtInscricaoDoCliente.Text = objLaudo.Cliente.InscricaoEstadual
            'Pedido
            txtPedido.Text = objLaudo.Pedido.Codigo
            imgExtratoPedido.Visible = True
            'CarregarOperacao()

            If objLaudo.CodigoOperacao > 0 Then ddlOperacao.SelectedValue = objLaudo.CodigoOperacao

            txtProduto.Text = objLaudo.CodigoProduto & "-" & objLaudo.Produto.Descricao

            If objLaudo.CodigoSubOperacao > 0 Then
                objLaudo.Consolidado = True
                Dim dsSaldoPedido As DataSet = objLaudo.SaldoDePedidos(objLaudo, False)

                If dsSaldoPedido.Tables(0).Rows.Count > 0 Then
                    If objLaudo.SubOperacao.Devolucao Then
                        hdnSaldoPedido.Value = dsSaldoPedido.Tables(0).Rows(0).Item("SaldoDevolucao")
                    Else
                        hdnSaldoPedido.Value = dsSaldoPedido.Tables(0).Rows(0).Item("Saldo")
                    End If
            ...
            hdnAutorizacaoDeRetirada.Value = objLaudo.CodigoAutorizacao
            txtSafra.Text = objLaudo.Pedido.CodigoSafra

            'Depositante
            txtDepositante.Text = objLaudo.Depositante.CodigoFormatado & "-" & objLaudo.Depositante.Nome
            hdnCodigoEndDepositante.Value = objLaudo.Depositante.CodigoEndereco

            'Depósito
            txtDeposito.Text = objLaudo.Deposito.CodigoFormatado & "-" & objLaudo.Deposito.Nome
            hdnCodigoEndDeposito.Value = objLaudo.Deposito.CodigoEndereco

            'Transportador
            If objLaudo.CodigoTransportador.Length > 0 Then
                txtTransportador.Text = objLaudo.Transportador.CodigoFormatado & "-" & objLaudo.Transportador.Nome
            ...
            'Classificação
            CarregarTabelaClassificacao()
            ddlTabelaDeClassificacao.SelectedValue = objLaudo.CodigoTabelaDeClassificacao
            'Análises
            ...
            If (txtSegundaPesagem.Text <> "0") Then
                ...
                If objLaudo.TemNota Then txtObservacao.ReadOnly = True
            ...
            Else
                ...
                'Desnecessario
                'Dim Analises = objLaudo.Analises
                ...
                If (objLaudo.Analises.Count > 1) Then
                ...
                If (gridDescontos.Rows.Count > 0) Then
                    If Len(objLaudo.Analises(0).Analise.Opcao) > 0 Then
            ...
            'Devolução ????
            'NOTA FISCAL PRODUTOR
            If objLaudo.NumeroDaNota Then
                txtNotaProdutor.Text = objLaudo.NumeroDaNota
                txtSerieNotaProdutor.Text = objLaudo.SerieDaNota
                txtPesoNotaProdutor.Text = objLaudo.PesoFiscal
                If objLaudo.SegundaPesagem > 0 Then
            ...
            If objLaudo.SegundaPesagem > 0 Then
                For Each objSubOperacao As SubOperacao In subOperacoes.FindAll(Function(p) p.EntradaSaida = entradaSaida)
                    ddlSubOperacao.Items.Add(New ListItem(objSubOperacao.CodigoOperacao.ToString("00") & "-" & objSubOperacao.Codigo.ToString("00") & " " & objSubOperacao.Descricao, _
            ...
            If objLaudo.CodigoSubOperacao > 0 Then
                Dim intIndice As Integer = ddlSubOperacao.Items.IndexOf(ddlSubOperacao.Items.FindByValue(objLaudo.CodigoOperacao & "-" & objLaudo.CodigoSubOperacao))
                If Not intIndice = -1 Then
                    ddlSubOperacao.SelectedIndex = intIndice
                End If
            End If
            ...
            If objLaudo.SegundaPesagem > 0 Then
                lblEntradaSaida.Text = IIf(objLaudo.EntradaSaida = "E", "ENTRADA", "SAIDA")
            ElseIf objLaudo.SubOperacao.EntradaSaida = eEntradaSaida.Entrada Then
                lblEntradaSaida.Text = "ENTRADA"
            ElseIf objLaudo.SubOperacao.EntradaSaida = eEntradaSaida.Saida Then
                lblEntradaSaida.Text = "SAIDA"
            End If

            'HISTÓRICO
            lblHistorico.Text = "Inclusão: " & objLaudo.Movimento.ToString("dd/MM/yyyy") & " - " & objLaudo.UsuarioInclusao.ToString() & "    " & IIf(Not String.IsNullOrWhiteSpace(objLaudo.UsuarioAlteracao), "Alteração: " & objLaudo.DataAlteracao & " - " & objLaudo.UsuarioAlteracao.ToString(), "") & _
    ...
    Protected Sub CalcularDesconto()
        ...
        objLaudo.BrutoBalanca = txtPesoBruto.Text
        objLaudo.CodigoProduto = txtProduto.Text.Split("-").ToArray(0).Trim

        If (Convert.ToInt32(txtPrimeiraPesagem.Text) > Convert.ToInt32(txtSegundaPesagem.Text)) Then
            objLaudo.EntradaSaida = "E"
        Else
            objLaudo.EntradaSaida = "S"
        End If
        ...
        For i = 0 To gridDescontos.Rows.Count - 1
            ...
            If Analis.Analise.Opcao.Length = 0 Then
                ...
                Dim reclassificar As Boolean = False

                If CDec(Percentual) = 0 Then
                    If Analis.CodigoAnalise = 1 Then
                        MsgBox(Me.Page, "Percentual da Umidade não pode ser Zero")
                        reclassificar = True
                    ElseIf Analis.CodigoAnalise = 2 Then
                        MsgBox(Me.Page, "Percentual da Impureza não pode ser Zero")
                        reclassificar = True
                    ElseIf Analis.CodigoAnalise = 3 Then
                        MsgBox(Me.Page, "Percentual do Avariados não pode ser Zero")
                        reclassificar = True
                    End If
                End If

                If reclassificar Then
                    For j = 0 To gridDescontos.Rows.Count - 1
                        CType(gridDescontos.Rows(j).FindControl("txtPercentual"), TextBox).Enabled = True
                        CType(gridDescontos.Rows(j).FindControl("txtIndice"), TextBox).Enabled = False
                        CType(gridDescontos.Rows(j).FindControl("txtDesconto"), TextBox).Enabled = False
                    Next
        ...
        If (String.IsNullOrEmpty(Erro)) Then
            ...
            txtLiquido.Text = objLaudo.BrutoBalanca - pDesconto
            ...
            btnIncluir.Enabled = True
            If objLaudo.EntradaSaida = "S" AndAlso _
                Not String.IsNullOrWhiteSpace(txtSegundaPesagem.Text) AndAlso _
    ...
    Public Sub Imprimir(ByVal Tipo As String, ByVal NrCopias As String, ByVal Impressao As String, ByRef objLaudo As [Lib].Negocio.Pesagem)
        Dim ds As New DataSet
        Dim i As Integer
        Dim myDataRow As DataRow

        sql = "SELECT Pesagem.Pesagem_Id AS Laudo, Pesagem.Produto, Pesagem.Placa, Pesagem.EntradaSaida, Pesagem.PrimeiraPesagem, " & vbCrLf & _
              "		  Pesagem.SegundaPesagem, Pesagem.BrutoBalanca, Pesagem.BrutoBalanca - Pesagem.Liquido AS Descontos, Pesagem.Liquido, Pesagem.EntradaPatio, " & vbCrLf & _
              "		  Pesagem.EntradaBalanca, Pesagem.SaidaBalanca, Pesagem.Movimento, Pesagem.NumeroDaNota AS NotaFiscal, Pesagem.SerieDaNota AS SerieNota, " & vbCrLf & _
              "		  Pesagem.PesoFiscal, Pesagem.Observacoes, Produtos.Nome AS NomeProduto, Clientes.Cliente_Id AS CodigoCliente, Clientes.Endereco_Id AS EndCliente, " & vbCrLf & _
              "		  Clientes.Nome AS NomeCliente, Clientes.Reduzido AS ReduzidoCliente, Clientes.Endereco AS EnderecoCliente, Clientes.Cidade AS CidadeCliente, " & vbCrLf & _
              "		  Clientes.Estado AS EstadoCliente, isnull(Transportes.Cliente_Id,'') AS CodigoTransportador, isnull(Transportes.Endereco_Id,0) AS EndTransportador, " & vbCrLf & _
              "       isnull(Transportes.Nome,'') AS NomeTransportador, isnull(Transportes.Reduzido,'') AS ReduzidoTransportador, isnull(Transportes.Endereco,'') AS EnderecoTransportador, " & vbCrLf & _
              "       isnull(Transportes.Cidade,'') AS CidadeTransportador, isnull(Transportes.Estado,'') AS EstadoTransportador, " & vbCrLf & _
              "		  Depositos.Cliente_Id AS CodigoDeposito, Depositos.Endereco_Id AS EndDeposito, Depositos.Nome AS NomeDeposito, Depositos.Reduzido AS ReduzidoDeposito, " & vbCrLf & _
              "       Depositos.Endereco AS EnderecoDeposito, Depositos.Cidade AS CidadeDeposito, Depositos.Estado AS EstadoDeposito, Depositos.Inscricao AS InscricaoDeposito, " & vbCrLf & _
              "		  isnull(Placas.Placa01,'') AS Placa01, isnull(Placas.Placa02,'') AS Placa02, isnull(Placas.Placa03,'') AS Placa03, isnull(Placas.CidadePlaca,'') AS CidadePlaca, isnull(Placas.EstadoPlaca,'') AS EstadoPlaca, " & vbCrLf & _
              "		  ISNULL(Motorista.Nome,'') AS NomeMotorista, ISNULL(Motorista.Cidade,'') AS CidadeMotorista, ISNULL(Motorista.Estado,'') AS EstadoMotorista, " & vbCrLf & _
              "		  ISNULL(Motorista.Habilitacao,'') AS Habilitacao, " & vbCrLf & _
              "		  CASE " & vbCrLf & _
              "		  	WHEN LEN(ISNULL(Motorista.Cliente_Id,'')) = 0 " & vbCrLf & _
              "		  	  THEN '' " & vbCrLf & _
              "		  	  ELSE CASE " & vbCrLf & _
              "		  			 WHEN LEN(Motorista.Cliente_Id) < 11 " & vbCrLf & _
              "		  			   THEN '' " & vbCrLf & _
              "		  			   ELSE SUBSTRING(Motorista.Cliente_Id, 1, 3) + '.' + SUBSTRING(Motorista.Cliente_Id, 4, 3) + '.' + SUBSTRING(Motorista.Cliente_Id, 7, 3) + '-' + SUBSTRING(Motorista.Cliente_Id, 10, 2) " & vbCrLf & _
              "		  			END " & vbCrLf & _
              "		  END AS CpfMotorista, " & vbCrLf & _
              "		  isnull(EstadoPlacaMotorista.Descricao,'') AS NomeEstadoMotorista, " & vbCrLf & _
              "		  isnull(EstadoPlaca.Descricao,'') AS NomeEstadoPlaca, " & vbCrLf & _
              "		  Pedido, Depositos.Numero AS NumeroDeposito, " & vbCrLf & _
              "		  Depositos.Complemento AS ComplementoDeposito, Depositos.Bairro AS BairroDeposito, Clientes.Numero AS NumeroCliente, Clientes.Complemento AS ComplementoCliente, " & vbCrLf & _
              "		  Clientes.Bairro AS BairroCliente, Clientes.Inscricao AS InscricaoCliente, Empresa.Cliente_Id  AS CodigoEmpresa, Empresa.Endereco_Id AS EndEmpresa, " & vbCrLf & _
              "		  Empresa.Nome AS NomeEmpresa, Empresa.Reduzido AS ReduzidoEmpresa, Empresa.Endereco AS EnderecoEmpresa, Empresa.Cidade  AS CidadeEmpresa, " & vbCrLf & _
              "		  Empresa.Estado AS EstadoEmpresa, Empresa.Inscricao AS InscricaoEmpresa, Empresa.Numero AS NumeroEmpresa, Empresa.Complemento AS ComplementoEmpresa, " & vbCrLf & _
              "		  Empresa.Bairro AS BairroEmpresa, isnull(rXp.Romaneio_id,0) AS Romaneio " & vbCrLf & _
              "  FROM Pesagem " & vbCrLf & _
              "	INNER JOIN Produtos " & vbCrLf & _
              "	   ON Pesagem.Produto = Produtos.Produto_Id " & vbCrLf & _
              "	INNER JOIN Clientes " & vbCrLf & _
              "	   ON Pesagem.Cliente = Clientes.Cliente_Id " & vbCrLf & _
              "	  AND Pesagem.EndCliente = Clientes.Endereco_Id " & vbCrLf & _
              "	 LEFT JOIN Clientes AS Transportes " & vbCrLf & _
              "	   ON Pesagem.Transportador = Transportes.Cliente_Id " & vbCrLf & _
              "	  AND Pesagem.EndTransportador = Transportes.Endereco_Id " & vbCrLf & _
              "  LEFT JOIN Clientes AS Motorista " & vbCrLf & _
              "	   ON Pesagem.Motorista = Motorista.Cliente_Id " & vbCrLf & _
              "	  AND Pesagem.EndMotorista = Motorista.Endereco_Id " & vbCrLf & _
              ...
              "	   ON Pesagem.Deposito = Depositos.Cliente_Id " & vbCrLf & _
              "	  AND Pesagem.EndDeposito = Depositos.Endereco_Id " & vbCrLf & _
              "	 LEFT JOIN Placas " & vbCrLf & _
              "	   ON Pesagem.Placa = Placas.Placa_Id " & vbCrLf & _
              "	 LEFT JOIN Estados AS EstadoPlaca " & vbCrLf & _
              "	   ON Placas.EstadoPlaca = EstadoPlaca.Estado_Id " & vbCrLf & _
              "	Inner Join Clientes as Empresa " & vbCrLf & _
              "	   on Empresa.cliente_id = Pesagem.Empresa_id " & vbCrLf & _
              "	  and Empresa.endereco_id = Pesagem.EndEmpresa_id " & vbCrLf & _
              "	 LEFT JOIN RomaneiosXPesagens rXp " & vbCrLf & _
              "	   on rXp.Empresa_id     = Pesagem.Empresa_id " & vbCrLf & _
              "	  and rXp.EndEmpresa_Id = Pesagem.EndEmpresa_id " & vbCrLf & _
              "	  and rXp.Pesagem_id    = Pesagem.Pesagem_id " & vbCrLf & _
              "	  and rXp.Sequencia_Id  = Pesagem.Sequencia_Id " & vbCrLf & _
              " WHERE (Pesagem.Empresa_Id = '" & objLaudo.CodigoEmpresa & "') " & vbCrLf & _
              "   AND (Pesagem.EndEmpresa_Id = " & objLaudo.EnderecoEmpresa & ") " & vbCrLf & _
              "   AND (Pesagem.Pesagem_Id = " & objLaudo.Codigo & ") " & vbCrLf & _
              "   AND (Pesagem.Sequencia_Id = 0)"

        ds = Banco.ConsultaDataSet(sql, "Laudo")


        sql = "Select CLA.Analise_Id AS Analise, " & vbCrLf & _
              "       case " & vbCrLf & _
              "         when Len(isnull(Analises.Opcao,'')) > 0 " & vbCrLf & _
              "           then Analises.Descricao + ' - ' + (select valor from fnStringToArray(Analises.Opcao,';') where left(valor,len(convert(int, pxa.percentual))+1) = convert(varchar,convert(int,pxa.percentual)) + '-')" & vbCrLf & _
              "           else Analises.Descricao " & vbCrLf & _
              "       end AS Descricao, " & vbCrLf & _
              "       isnull(pxa.percentual,0) as Percentual," & vbCrLf & _
              ...
              "	   on p.Empresa_Id      ='" & objLaudo.CodigoEmpresa & "'" & vbCrLf & _
              "	  and p.EndEmpresa_Id   = " & objLaudo.EnderecoEmpresa & vbCrLf & _
              "	  and p.Pesagem_Id      = " & objLaudo.Codigo & vbCrLf & _
              "   and p.sequencia_id    = " & objLaudo.Sequencia & vbCrLf & _
              "  Left Join PesagemXAnalises pxa" & vbCrLf & _
              "    ON pxa.Empresa_Id    = p.Empresa_Id " & vbCrLf & _
              "   AND pxa.EndEmpresa_Id = p.EndEmpresa_Id " & vbCrLf & _
              "   AND pxa.Pesagem_Id    = p.Pesagem_Id " & vbCrLf & _
              "   AND pxa.Sequencia_Id  = p.Sequencia_Id " & vbCrLf & _
              "   And pxa.Analise_id    = cla.Analise_Id" & vbCrLf & _
              "  Where CLA.Tabela_id    = " & objLaudo.CodigoTabelaDeClassificacao & vbCrLf & _
              "    and CLA.Produto_id   ='" & objLaudo.CodigoProduto & "'" & vbCrLf & _
    ...
    Public Sub Incluir()
        SessaoRecuperaLaudo()
        'dados da pesagem
        objLaudo.IUD = "I" 'Insert
        objLaudo.Sequencia = 0 'inicia com zero na inclusão.
        objLaudo.EntradaPatio = DateTime.Now
        objLaudo.EntradaBalanca = DateTime.Now
        objLaudo.UsuarioInclusao = Session("ssNomeUsuario").ToString
        objLaudo.Analises = Nothing
        objLaudo.CodigoSituacao = ddlSituacao.SelectedValue
        objLaudo.CodigoOperacao = ddlOperacao.SelectedValue.ToString
        objLaudo.CodigoSubOperacao = ddlSubOperacao.SelectedValue.Split("-").ToArray(1).Trim 'dúvida do pedido ou pode ser alterado?
        objLaudo.PrimeiraPesagem = CType(txtPrimeiraPesagem.Text, Decimal)
        objLaudo.PrimeiraPesagem = txtPrimeiraPesagem.Text
        objLaudo.Movimento = txtMovimento.Text
        'Nota Produtor
        If (Not String.IsNullOrEmpty(txtNotaProdutor.Text)) Then
            objLaudo.NumeroDaNota = txtNotaProdutor.Text
            objLaudo.SerieDaNota = txtSerieNotaProdutor.Text
        ...
        End If
        objLaudo.Observacoes = Funcoes.EliminarCaracteresEspeciais(txtObservacao.Text)
        objLaudo.TemRomaneio = False 'Default(N) Não utilizado na pesagem
        objLaudo.RegistroMestre = 0 'Sempre 0
        objLaudo.PesagemDeTerceiros = 0 'Default(0)
    ...
    Protected Sub imgImprimir_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs)
        If Funcoes.VerificaPermissao("LaudoDePesagem", "RELATORIO") Then
            ...
            objLaudo = New Pesagem(gdvLaudo.Rows(row.RowIndex).Cells(2).Text, gdvLaudo.Rows(row.RowIndex).Cells(3).Text, gdvLaudo.Rows(row.RowIndex).Cells(1).Text)
            SessaoSalvaLaudo()
            If objLaudo.SegundaPesagem > 0 AndAlso objLaudo.Analises IsNot Nothing Then
                Imprimir("2", txtNumeroDeCopias.Text, "A", objLaudo)
            Else
    ...
    Public Sub Cancelar()
        ...
        For Each ron As [Lib].Negocio.Romaneio In objLaudo.Romaneios
            ron.IUD = "D"
        Next
