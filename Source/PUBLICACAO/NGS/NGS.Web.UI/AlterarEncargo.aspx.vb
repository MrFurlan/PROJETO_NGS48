...
Public Class AlterarEncargo
    ...
    Private Sub AtualizarEncargos(ByVal index As Integer)
        ...
        If objNotaFiscal.NFG Then
            ...
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "FACS" OrElse
               ...
               objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "IAGRO" OrElse
            ...
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "DESCONTOS" Then objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Encargo.Atualizacao = True
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "DESP. ACESSORIA" Then objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Encargo.Atualizacao = True
            ...
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).ValorPeso = eValorPeso.Peso Then
                If Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "FACS" And Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "FETHAB" And Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "IAGRO" Then
            ...
            Else
                If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Base = 0 OrElse
                    ...
                    If Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo.Contains("FGTS") AndAlso
                       Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo.Contains("FRETES") AndAlso
                       Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo.Contains("DESCONTOS") AndAlso
                       Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo.Contains("DESP. ACESSORIA") AndAlso
                       Not objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo.Contains("ICMS MONO") AndAlso
            ...
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "DESCONTOS" Then objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Encargo.Atualizacao = False
            If objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Codigo = "DESP. ACESSORIA" Then objNotaFiscal.Itens.Where(Function(s) s.IUD <> "D")(idGridItem.Value).Encargos(index).Encargo.Atualizacao = False
    ...
    Private Sub IniciarAlteracaoNotaFiscal()
        ...
        Else
            ...
            If objNotaFiscal.CodigoTipoDeDocumento = 57 Then

                If original.TotalNota = objNotaFiscal.TotalNota Then

                    Dim Sqls As New ArrayList

                    original.IUD = "D"
                    original.CarregandoNota = True
                    original.Itens(idGridItem.Value).IUD = "D"
                    original.Itens(idGridItem.Value).Encargos.SalvarSql(Sqls)

                    original.Razao.ExcluiContabilizacaoNotaSQL(Sqls, 9)
                    original.Razao.ExcluiContabilizacaoNotaSQL(Sqls, 10)
                    original.Razao.ExcluiContabilizacaoNotaSQL(Sqls, 11)
                    original.Razao.ExcluiContabilizacaoNotaSQL(Sqls, 21)

                    Dim obs As String = objNotaFiscal.ObservacoesControleInterno
                    If Not String.IsNullOrWhiteSpace(obs) AndAlso obs.Length > 0 Then
                        obs = obs & ". Ajustado em AlterarEncargo em " & Format(Now, "dd/MM/yyyy HH:mm:ss") & " feito por " & HttpContext.Current.Session("ssNomeUsuario")
                    Else
                        obs = "Ajustado em AlterarEncargo em " & Format(Now, "dd/MM/yyyy HH:mm:ss") & " feito por " & HttpContext.Current.Session("ssNomeUsuario")
                    End If
                    ...
                    objNotaFiscal.Itens(idGridItem.Value).IUD = "I"
                    objNotaFiscal.Itens(idGridItem.Value).Encargos.SalvarSql(Sqls)

                    objNotaFiscal.Razao.ContabilizarNotaSql(Sqls, 21)

                    If Sqls.Count > 0 Then
                        If Banco.GravaBanco(Sqls) Then
                            MsgBox(Me.Page, "Conhecimento ajustado com sucesso.")
                            Limpar()
                        Else
                            MsgBox(Me.Page, "Erro ao Alterar o CTe: " & Funcoes.EliminarCaracteresEspeciais(RTrim(HttpContext.Current.Session("ssMessage" & HID.Value).ToString)) & ". Entre em contato com o Suporte de TI")
                        End If
                    End If

                    Exit Sub
                Else
                    MsgBox(Me.Page, "Conhecimento não pode ser ajustado por aqui.")
                    Exit Sub
                End If
            End If
            ...
            Else
                ...
                If Banco.GravaBanco(Sqls) Then
                    MsgBox(Me.Page, "Nota Fiscal ajustada com sucesso.")
                    Limpar()
                Else
                    MsgBox(Me.Page, "Erro ao Alterar a Nota: " & Funcoes.EliminarCaracteresEspeciais(RTrim(HttpContext.Current.Session("ssMessage" & HID.Value).ToString)) & ". Entre em contato com o Suporte de TI")
                End If
    ...
    Protected Sub lnkConsultar_Click(sender As Object, e As EventArgs) Handles lnkConsultar.Click
        Try
            ...
            Session("ssCampo" & HID.Value) = "AlterarEncargos"
